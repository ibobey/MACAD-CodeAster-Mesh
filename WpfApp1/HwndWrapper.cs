...
namespace CodeAsterMesh
{
    ...
    public abstract class HwndWrapper : HwndHost
    {
        ...
        protected override IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            switch (msg)
            {
                case NativeMethods.WM_MOUSEWHEEL:
                    if (_mouseInWindow)
                    {
                        int delta = NativeMethods.GetWheelDeltaWParam(wParam);
                ...
                case NativeMethods.WM_LBUTTONDBLCLK:
                    RaiseHwndLButtonDblClick(new HwndMouseEventArgs(_mouseState, 0, MouseButton.Left));
                    break;
                ...
                case NativeMethods.WM_RBUTTONDBLCLK:
                    RaiseHwndRButtonDblClick(new HwndMouseEventArgs(_mouseState, 0, MouseButton.Right));
                    break;
                ...
                case NativeMethods.WM_MBUTTONDBLCLK:
                    RaiseHwndMButtonDblClick(new HwndMouseEventArgs(_mouseState, 0, MouseButton.Middle));
                    break;
                ...
                case NativeMethods.WM_XBUTTONDBLCLK:
                    if (((int)wParam & NativeMethods.MK_XBUTTON1) != 0)
                        RaiseHwndX1ButtonDblClick(new HwndMouseEventArgs(_mouseState, 0, MouseButton.XButton1));
                    else if (((int)wParam & NativeMethods.MK_XBUTTON2) != 0)
                        RaiseHwndX2ButtonDblClick(new HwndMouseEventArgs(_mouseState, 0, MouseButton.XButton2));
                    break;
                ...
                case NativeMethods.WM_SIZE:
                    // lParam contains the new width in the low-order word and height in the high-order word.
                    int newWidth = NativeMethods.GetXLParam((int)lParam); // Or LOWORD macro
                    int newHeight = NativeMethods.GetYLParam((int)lParam); // Or HIWORD macro

                    // This WndProc is part of HwndWrapper, which doesn't know about _occtView directly.
                    // You would need a way for HwndWrapper to notify its derived class (OcctHwndControl)
                    // or OcctHwndControl would need to override WndProc.

                    // If OcctHwndControl overrides WndProc:
                    NativeResized(newWidth, newHeight);
                    // IMPORTANT: You usually want to call base.WndProc for WM_SIZE so HwndHost can do its thing.
                    // So, you might not set 'handled = true' or call base first.
                    break;
            }
        ...
        protected abstract void NativeResized(int newWidth, int newHeight);
